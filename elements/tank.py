...
class Tank:
    def __init__(self, fluid: Oxidizer, environment: Environment, volume: float, loaded_fluid_mass: float):
        ...
        self.quantity_gaseous = vapor_pressure_gaseous * \
            (self.volume - molar_volume_liquid * self.total_N2O) / denominator              # [mol]
        self.quantity_liquid = (self.total_N2O * environment.R * fluid.temperature - \
                                vapor_pressure_gaseous * self.volume) / denominator         # [mol]
    ...
    def calculate_tank_derivatives(self, oxidizer_mass_flow: float) -> None:
        ...
        if(fluid.phase == Phase.LIQUID):
            ...
            self.delta_temperature = fluid.temperature * \
                (self.fluid.get_specific_heats_ratio(fluid.phase) - 1) * f / self.quantity_gaseous
